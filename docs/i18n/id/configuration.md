...
For Azure resource setup, see [Penyiapan Azure AI](azure-ai-setup.md).
...
| --- | --- | --- | --- |
| `translate -md` | Ya | Tidak | Menerjemahkan hanya Markdown. |
| `translate -nb` | Ya | Tidak | Menerjemahkan hanya notebook. |
| `translate -img` | Ya | Ya | Menerjemahkan hanya gambar. |
| `translate` with no type flags | Ya | Ya | Mode bawaan mencakup Markdown, notebook, dan gambar. |
| `evaluate` | Ya | Tidak | Menggunakan evaluasi LLM kecuali `--fast` dipilih. |
| `migrate-links` | Ya | Tidak | Melakukan migrasi tautan, tetapi tetap menjalankan pemeriksaan konfigurasi bersama. |
| `co-op-review` | Tidak | Tidak | Menjalankan pemeriksaan deterministik struktur terjemahan, kesegaran, Markdown, notebook, dan tautan lokal. |
| `run_translation(markdown=True)` | Ya | Tidak | Terjemahan Markdown secara programatis. |
| `run_translation(images=True)` | Ya | Ya | Terjemahan gambar secara programatis. |
