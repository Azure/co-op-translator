...
| `translations_dir` | `str \| None` | `None` | Vlastný výstupný adresár pre preklady textu. Relatívne cesty sa riešia voči každému koreňu. |
| `root_dirs` | `Iterable[str] \| None` | `None` | Viaceré korene, ktoré zdieľajú rovnaké výstupné nastavenia. |
| `groups` | `Iterable[tuple[str, str \| None]] \| None` | `None` | Explicitné `(root_dir, translations_dir)` páry. Majú prednosť pred `root_dirs`. |
| `changed_from` | `str \| None` | `None` | Git ref použitý na obmedzenie kontroly na zmenené zdrojové súbory. |
| `output_format` | `str` | `"text"` | Formát výstupu kontroly. Podporované hodnoty sú `"text"` a `"github"`. |
...
Povinné premenné pre Azure OpenAI:
...
Povinné premenné Azure AI Vision pre preklad obrázkov:
...
`run_review` je deterministický a nevyžaduje konfiguráciu Azure OpenAI, OpenAI ani Azure AI Vision.
...
- `run_review` predvolene hlási chýbajúce lokálne Markdown a ciele odkazov na obrázky ako varovania.

## Interná cesta volaní
...
| --- | --- | --- |
| `ProjectTranslator` | `co_op_translator.core.project.project_translator` | Koordinuje preklad na úrovni projektu, správu adresárov, normalizáciu metadát pre jednotlivé jazyky a delegovanie na prekladače Markdown, notebookov a obrázkov. |
| `TranslationManager` | `co_op_translator.core.project.translation` | Vykonáva asynchrónne spracovanie súborov pre Markdown, notebooky, obrázky, detekciu zastaranosti a aktualizácie metadát prekladu. |
| `ProjectMarkdownTranslationMixin` | `co_op_translator.core.project.translation.project_markdown_translation` | Orchestruje čítanie Markdown súborov, preklad obsahu, prepísanie ciest, metadáta, upozornenia a zápisy. |
| `ProjectNotebookTranslationMixin` | `co_op_translator.core.project.translation.project_notebook_translation` | Orchestruje čítanie notebookov, preklad buniek Markdown, prepísanie ciest, metadáta, upozornenia a zápisy. |
| `ProjectImageTranslationMixin` | `co_op_translator.core.project.translation.project_image_translation` | Orchestruje zisťovanie zdrojových obrázkov, preklad obrázkov, výstupné cesty, metadáta a zápisy. |
| `ProjectEvaluator` | `co_op_translator.core.project.project_evaluator` | Nájde preložené páry Markdown, vyhodnocuje kvalitu prekladu a číta metadáta dôveryhodnosti pre pracovné postupy opravy s nízkou dôverou. |
| `ReviewRunner` | `co_op_translator.review.runner` | Koordinuje deterministické kontrolné overenia naprieč zdrojovými súbormi, cieľovými jazykmi a nakonfigurovanými koreňmi prekladu. |
| `ReviewTarget` | `co_op_translator.review.targets` | Popisuje zdrojový koreň a výstupný adresár prekladu, ktorý sa pre tento koreň kontroluje. |
| `LanguageFolderMigrator` | `co_op_translator.core.project.language_migrator` | Detekuje staršie aliasové jazykové priečinky a pripravuje plány migrácie na kanonické názvy priečinkov podľa BCP 47. |
| `Config` | `co_op_translator.config.base_config` | Načíta súbory `.env` a skontroluje, či sú nakonfigurovaní povinní LLM a voliteľní Vision poskytovatelia. |
| `LLMConfig` | `co_op_translator.config.llm_config.config` | Automaticky detekuje Azure OpenAI alebo OpenAI, overí povinné premenné prostredia a vykoná kontroly konektivity poskytovateľa. |
| `VisionConfig` | `co_op_translator.config.vision_config.config` | Detekuje konfiguráciu Azure AI Vision a vykoná kontroly konektivity pre preklad obrázkov. |