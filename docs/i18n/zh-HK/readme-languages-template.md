...
Co-op Translator 可以為發佈已翻譯內容的儲存庫維護 README 語言表。

當你希望 Co-op Translator 在每次翻譯執行時取代整個區段，請使用下面的標記。若你偏好手動維護自訂的子集，請移除這些標記。
...
在翻譯期間自訂 sparse-checkout 儲存庫的 URL：

```bash
translate -l "ko" --repo-url "https://github.com/org/repo.git"
```

如果這些標記存在，當新增或變更語言時，Co-op Translator 可以更新生成的表格。