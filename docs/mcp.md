...
This dry run uses local discovery and estimation. It does not require provider
credentials, run connectivity checks, or write translation, log, or event files.
...
to an NDJSON file when `dry_run=false`. During a dry run, use the returned
