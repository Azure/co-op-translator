...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-03T07:07:47+00:00",
...
1. 루트 디렉토리에 테스트 디렉토리를 만듭니다:
    ```bash
    mkdir test_docs
    ```

2. 번역하고자 하는 일부 마크다운 문서와 이미지를 테스트 디렉토리에 복사합니다. 예를 들어:
    ```bash
...
1. 제공된 `.env.template` 파일을 복사하여 루트 디렉토리에 `.env` 파일을 만듭니다.
...
> [!TIP]
> 코드 저장 시 Black으로 자동 포맷되도록 편집기를 설정하는 것이 좋습니다. 대부분의 현대 편집기는 확장이나 플러그인을 통해 이를 지원합니다.

## Co-op Translator 실행

환경에서 Poetry를 사용하여 Co-op Translator를 실행하려면 다음 단계를 따르세요:

1. 번역 테스트를 수행할 디렉토리로 이동하거나 테스트 목적으로 임시 폴더를 만듭니다.

2. 다음 명령을 실행합니다. `-l ko`를 번역하려는 언어 코드로 바꿉니다. `-d` 플래그는 디버그 모드를 나타냅니다.
...
- **type**: 커밋의 범주를 지정합니다. 다음 유형을 사용합니다:
  ...
  - `Core`: 프로젝트의 핵심 기능 또는 기능 수정, 특히 `src/co_op_translator/core` 디렉토리의 파일과 관련된 것.
...
이들은 다음과 같이 압축되어야 합니다:
`Docs: 문서 명확성과 형식 개선 (#65)`
