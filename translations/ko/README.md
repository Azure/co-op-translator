...
{
  "original_hash": "4456727386868219b28a00c6399dd6f2",
  "translation_date": "2025-08-10T12:10:59+00:00",
...
_여러 언어로 문서를 쉽게 번역하여 전 세계의 독자에게 다가가세요._

### Co-op Translator가 제공하는 언어 지원
...
**Co-op Translator**는 Microsoft의 대규모 교육 시리즈(예: "초보자를 위한" 가이드)의 비효율적인 수동 번역 프로세스를 해결하기 위해 탄생했습니다. 이 도구는 모든 사람을 위한 장벽을 허물기 위해 설계된 사용하기 쉽고 강력한 도구로 발전했습니다. CLI 및 GitHub Actions를 통해 고품질의 자동 번역을 제공함으로써 Co-op Translator는 전 세계의 교육자, 학생, 연구자 및 개발자가 언어의 제약 없이 지식을 공유하고 접근할 수 있도록 지원합니다.
...
Co-op Translator는 글로벌 개발자 커뮤니티에 서비스를 제공하는 리포지토리의 번역 프로세스를 자동화하여 Microsoft의 주요 교육 이니셔티브의 언어 격차를 해소하는 데 도움을 줍니다. 현재 Co-op Translator를 사용하는 예로는 다음과 같은 것들이 있습니다:
...
- **이미지 텍스트 번역**: 이미지 내의 텍스트를 추출하고 번역합니다.
...
![아키텍처](../../translated_images/architecture_241019.441572b69458c4120e51cfb1fe1f7ae760f58272f464ad161f1e654092cdec40.ko.png)

Co-op Translator는 프로젝트 폴더에서 Markdown 파일과 이미지를 가져와 다음과 같이 처리합니다:

1. **텍스트 추출**: Markdown 파일과, 설정된 경우(예: Azure AI Vision 사용), 이미지에 포함된 텍스트를 추출합니다.
...
2. **GitHub Actions** - 푸시할 때마다 자동 번역을 위한 방법
...
Co-op Translator는 전 세계의 독자에게 다가갈 수 있도록 다양한 언어를 지원합니다. 알아두어야 할 사항은 다음과 같습니다:
...
| 페르시아어 (파르시어) | fa | 폴란드어 | pl | 포르투갈어 (브라질) | br |
...
> [!NOTE]
> AI 비전 서비스가 사용 가능하지 않은 경우, 협업 번역기는 [Markdown-only 모드](./getting_started/markdown-only-mode.md)로 전환됩니다.
...
1. 언어 모델 리소스 (필수):
   - Azure OpenAI (추천) - 기업 수준의 신뢰성을 갖춘 고품질 번역 제공
   - OpenAI - Azure 접근이 없는 경우 대안 옵션
   - 지원되는 모델에 대한 자세한 정보는 [지원되는 모델 및 서비스](../..)를 참조하세요

1. AI 비전 리소스 (선택 사항):
   - Azure AI Vision - 이미지 내 텍스트 번역 가능
   - 설정되지 않은 경우, 번역기는 자동으로 [Markdown-only 모드](./getting_started/markdown-only-mode.md)를 사용합니다
   - 텍스트가 포함된 이미지를 번역해야 하는 프로젝트에 추천

1. 구성 단계:
   - 자세한 지침은 [Azure AI 설정 가이드](./getting_started/set-up-azure-ai.md)를 참조하세요
   - API 키와 엔드포인트를 포함한 `.env` 파일을 생성하세요 (자세한 내용은 [빠른 시작](../..) 섹션 참조)
   - 선택한 서비스에 필요한 권한과 할당량을 확인하세요

### 번역 전 프로젝트 설정

번역 프로세스를 시작하기 전에 프로젝트를 준비하기 위해 다음 단계를 따르세요:
...
1. 기존 번역 정리 (필요한 경우):
   ...
   - 오래된 번역 파일을 삭제하여 새로 시작하세요
   - 이는 새로운 번역 프로세스와의 충돌을 방지합니다
...
1. 번역 실행:
    ...
    _(원하는 공백으로 구분된 언어 코드를 `"ko ja fr"` 대신 입력하세요)_

### 상세 사용 가이드

워크플로우에 가장 적합한 접근 방식을 선택하세요:

#### 1. 명령줄 사용 (CLI)

- 적합한 경우: 일회성 번역, 수동 제어, 또는 사용자 정의 스크립트 통합.
- 필요 사항: Python의 로컬 설치 및 `co-op-translator` 패키지.
- 가이드: [명령줄 가이드](./getting_started/command-line-guide/command-line-guide.md)

#### 2. GitHub Actions 사용 (자동화)

- 적합한 경우: 저장소에 변경 사항이 푸시될 때마다 콘텐츠를 자동으로 번역. 번역을 지속적으로 최신 상태로 유지.
- 필요 사항: 저장소에 워크플로우 파일 (`.github/workflows`) 설정. 로컬 설치 필요 없음.
- 가이드:
  - [GitHub Actions 가이드 (공개 저장소 및 표준 비밀)](./getting_started/github-actions-guide/github-actions-guide-public.md) - 대부분의 공개 또는 개인 저장소에서 표준 저장소 비밀을 사용하는 경우 사용하세요.
...
- [명령 참조](./getting_started/command-reference.md): 사용 가능한 모든 명령 및 옵션에 대한 자세한 가이드.
- [지원되는 언어](./getting_started/supported-languages.md): 지원되는 언어 목록 및 새로운 언어 추가 지침 확인.
- [Markdown-Only 모드](./getting_started/markdown-only-mode.md): 이미지 번역 없이 텍스트만 번역하는 방법.

## 비디오 프레젠테이션

협업 번역기에 대한 프레젠테이션을 통해 더 알아보세요 _(아래 이미지를 클릭하여 YouTube에서 시청하세요.)_:

- **Microsoft에서 열기**: 협업 번역기를 사용하는 방법에 대한 간단한 18분 소개 및 빠른 가이드.
...
교육 콘텐츠가 전 세계적으로 공유되는 방식을 혁신하는 데 함께하세요! GitHub에서 [협업 번역기](https://github.com/azure/co-op-translator)에 ⭐를 주고 학습과 기술의 언어 장벽을 허물기 위한 우리의 사명을 지원하세요. 여러분의 관심과 기여는 큰 영향을 미칩니다! 코드 기여와 기능 제안은 언제나 환영합니다.

## 기여하기

이 프로젝트는 기여와 제안을 환영합니다. Azure 협업 번역기에 기여하는 데 관심이 있으신가요? 협업 번역기를 더 접근 가능하게 만드는 방법에 대한 지침은 [CONTRIBUTING.md](./CONTRIBUTING.md)를 참조하세요.
...
Microsoft는 고객이 AI 제품을 책임감 있게 사용할 수 있도록 돕고, 학습을 공유하며, 투명성 노트와 영향 평가와 같은 도구를 통해 신뢰 기반의 파트너십을 구축하는 데 전념하고 있습니다. 이러한 리소스의 많은 부분은 [https://aka.ms/RAI](https://aka.ms/RAI)에서 찾을 수 있습니다.
Microsoft의 책임 있는 AI 접근 방식은 공정성, 신뢰성 및 안전성, 개인정보 보호 및 보안, 포용성, 투명성, 책임성이라는 AI 원칙에 기반을 두고 있습니다.

이 샘플에서 사용되는 대규모 자연어, 이미지 및 음성 모델은 불공정하거나 신뢰할 수 없거나 공격적인 방식으로 작동할 수 있으며, 그 결과 해를 끼칠 수 있습니다. 위험과 제한 사항에 대한 정보를 얻으려면 [Azure OpenAI 서비스 투명성 노트](https://learn.microsoft.com/legal/cognitive-services/openai/transparency-note?tabs=text)를 참조하세요.

이러한 위험을 완화하는 권장 접근 방식은 해로운 행동을 감지하고 방지할 수 있는 안전 시스템을 아키텍처에 포함하는 것입니다. [Azure AI 콘텐츠 안전](https://learn.microsoft.com/azure/ai-services/content-safety/overview)은 애플리케이션 및 서비스에서 사용자 생성 및 AI 생성 콘텐츠의 해로운 내용을 감지할 수 있는 독립적인 보호 계층을 제공합니다. Azure AI 콘텐츠 안전에는 해로운 자료를 감지할 수 있는 텍스트 및 이미지 API가 포함되어 있습니다. 또한 다양한 모달리티에서 해로운 콘텐츠를 감지하기 위한 샘플 코드를 보고, 탐색하고, 시도할 수 있는 대화형 콘텐츠 안전 스튜디오도 제공합니다. 다음 [빠른 시작 문서](https://learn.microsoft.com/azure/ai-services/content-safety/quickstart-text?tabs=visual-studio%2Clinux&pivots=programming-language-rest)는 서비스에 요청을 보내는 방법을 안내합니다.

고려해야 할 또 다른 측면은 전체 애플리케이션 성능입니다. 다중 모달 및 다중 모델 애플리케이션에서 성능은 시스템이 사용자와 사용자가 기대하는 대로 작동하며, 해로운 출력을 생성하지 않는 것을 의미합니다. [생성 품질 및 위험 및 안전 메트릭](https://learn.microsoft.com/azure/ai-studio/concepts/evaluation-metrics-built-in)을 사용하여 전체 애플리케이션의 성능을 평가하는 것이 중요합니다.

개발 환경에서 AI 애플리케이션을 평가하려면 [프롬프트 흐름 SDK](https://microsoft.github.io/promptflow/index.html)를 사용할 수 있습니다. 테스트 데이터 세트 또는 목표를 제공하면 생성 AI 애플리케이션의 생성물이 내장 평가자 또는 선택한 사용자 정의 평가자로 정량적으로 측정됩니다. 시스템을 평가하기 위해 프롬프트 흐름 SDK를 시작하려면 [빠른 시작 가이드](https://learn.microsoft.com/azure/ai-studio/how-to/develop/flow-evaluate-sdk)를 따를 수 있습니다. 평가 실행을 수행한 후에는 [Azure AI 스튜디오에서 결과를 시각화](https://learn.microsoft.com/azure/ai-studio/how-to/evaluate-flow-results)할 수 있습니다.

## 상표

이 프로젝트에는 프로젝트, 제품 또는 서비스의 상표 또는 로고가 포함될 수 있습니다. Microsoft 상표 또는 로고의 허가된 사용은 [Microsoft의 상표 및 브랜드 지침](https://www.microsoft.com/en-us/legal/intellectualproperty/trademarks/usage/general)을 준수해야 합니다.
