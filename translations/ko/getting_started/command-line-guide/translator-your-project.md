...
**Co-op Translator**의 일반적인 사용 사례와 적절한 명령어를 소개합니다.

### 1. 기본 번역 (단일 언어)

프로젝트 전체(마크다운 파일 및 이미지)를 한국어와 같은 단일 언어로 번역하려면 다음 명령어를 사용하세요:
...
**Phi-3 CookBook**에서는 최신 커밋을 반영하기 위해 변경 사항을 가져온 후, 새로 추가된 마크다운 파일과 이미지를 번역하기 위해 다음 방법을 사용했습니다.
...
기존 번역을 업데이트하려면(즉, 현재 번역을 삭제하고 새로운 번역으로 교체) `-u` 옵션을 사용하세요. 이 옵션은 지정된 언어의 모든 기존 번역을 삭제하고 다시 번역합니다.
...
**Phi-3 CookBook**에서는 스페인어로 번역된 모든 파일을 업데이트하기 위해 다음 방법을 사용했습니다. 여러 마크다운 문서의 원본 내용에 큰 변화가 있을 때 이 방법을 사용하는 것이 좋습니다. 번역된 마크다운 파일이 몇 개만 업데이트되어야 한다면, 특정 파일을 수동으로 삭제한 후 `-a` 방법을 사용하여 업데이트된 번역을 추가하는 것이 더 효율적입니다.
...
번역된 파일의 오류를 확인하고 필요한 경우 번역을 다시 시도하려면 `-chk` 옵션을 사용하세요:
...
**Phi-3 CookBook**에서는 한국어 파일의 번역 오류를 확인하고 문제가 있는 파일에 대해 자동으로 번역을 다시 시도하기 위해 다음 방법을 사용했습니다.
...
이 옵션은 번역 오류를 확인합니다. 현재는 원본 파일과 번역된 파일의 줄 바꿈 차이가 6개 이상일 경우 번역 오류로 표시됩니다. 앞으로 더 유연한 기준을 위해 이 기준을 개선할 계획입니다.
...
문제 해결을 위한 자세한 로그를 활성화하려면 `-d` 옵션을 사용하세요:

```bash
translate -l "ko" -d
```

이 명령어는 번역을 디버그 모드로 실행하여 번역 과정에서 발생하는 문제를 식별하는 데 도움이 되는 추가 로그 정보를 제공합니다.
...
이 명령어는 프로젝트를 모든 사용 가능한 언어로 번역합니다. 진행할 경우 프로젝트 크기에 따라 번역에 상당한 시간이 걸릴 수 있습니다.
...
>
> 그러나 특정 파일을 다시 번역하거나 시스템 동작을 무시하려면, 다음 명령어를 사용하여 언어 폴더 전체에서 파일의 모든 버전을 삭제할 수 있습니다.
>
> ### Windows에서:
> 1. **명령 프롬프트 사용**:
>    - 명령 프롬프트를 엽니다.
...
>      del /s *filename*
>      ```
>      `filename`을 찾고자 하는 파일 이름의 특정 부분으로 바꿉니다. `/s` 옵션은 하위 디렉토리도 검색합니다.
>
> 2. **PowerShell 사용**:
>    - PowerShell을 엽니다.
>    - 다음 명령어를 실행합니다:
